...
const BodySchema = z.object({
  ...
  scheduledIntervalMinutes: IntervalSchema.default(60)
});

const AutomaticSyncBodySchema = z.object({
...
export async function PATCH(request: Request) {
  ...
  const parsed = AutomaticSyncBodySchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return fail("VALIDATION_ERROR", "Configuracao da sincronizacao automatica invalida.", 400);
  }

  try {
    const data = await setAutomaticSyncSettings(
      parsed.data.automaticSyncEnabled,
      parsed.data.scheduledIntervalMinutes,
      auth.profile.id
    );
    ...
    return ok(
      ...
      data.automaticSyncEnabled
        ...
        : "Sincronizacao automatica desativada. O processamento geral ocorrera somente quando iniciado manualmente."
    );
  } catch (error) {
    return fail(
      "DATABASE_ERROR",
      error instanceof Error
        ? error.message
        : "Nao foi possivel alterar a sincronizacao automatica.",
      500
    );
  }
}
