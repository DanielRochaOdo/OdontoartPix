...
export function AppShell({
  ...
  return (
    <div className="min-h-screen bg-app text-primary">
      ...
      >
        {!focusMode ? (
          <aside className="border-r border-[#1a6258] bg-[radial-gradient(circle_at_top,#0c332f_0%,#06201f_42%,#041817_100%)] text-white shadow-[8px_0_40px_rgba(0,0,0,0.18)] lg:sticky lg:top-0 lg:h-screen">
            <div className={`flex h-full max-h-screen flex-col overflow-hidden py-3 ${collapsed ? "px-2" : "px-4"}`}>
                ...
                <div className={`flex items-center ${collapsed ? "justify-center" : "gap-4 px-3"}`}>
                  ...
                  {!collapsed ? (
                    <div className="min-w-0">
                      <div className="text-xs uppercase tracking-[0.3em] text-[#67e4c8]">
                        OdontoartPix
                      </div>
                      <h1 className="mt-1 text-base font-medium leading-tight tracking-[0.08em] text-[#52cdb4]">
                ...
                {!collapsed ? (
                  <div className="mt-3 flex items-center gap-3 rounded-2xl border border-[#1b6259] bg-[#0a2d2a]/80 px-3 py-3 shadow-[inset_0_1px_0_rgba(117,255,222,0.04)]">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 truncate text-sm font-semibold text-[#f2fffc]">
                        {profile.email ?? profile.nome ?? "Usuário autenticado"}
                      </p>
                    </div>
              ...
              <nav className={`mt-5 flex-1 overflow-y-auto pr-1 ${collapsed ? "space-y-4" : "space-y-2"}`}>
                {navItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border transition ${
                        isActive
              ...
              <div
                className={`mt-4 shrink-0 border-t border-[#1b5b53] pt-3 ${
                  collapsed ? "flex flex-col items-center gap-3" : "space-y-3 px-1"
                }`}
              >
