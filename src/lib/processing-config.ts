...
export function getProcessingConfig(): ProcessingConfig {
  ...
  cachedConfig = {
    ...
    claimBatchSize: readInteger("PROCESSING_BLOCK_SIZE", 30, 1, 500),
    perWorkerConcurrency: readInteger("PROCESSING_CONCURRENCY", 10, 1, 100),
    ...
    workerCycleBudgetMs: readInteger("PROCESSING_WORKER_CYCLE_BUDGET_MS", 40000, 5000, 120000),
    globalLockLeaseSeconds: readInteger("PROCESSING_LEASE_SECONDS", 900, 30, 3600),
    productiveDelayMs: readInteger("PROCESSING_PRODUCTIVE_DELAY_MS", 25, 0, 10000),
