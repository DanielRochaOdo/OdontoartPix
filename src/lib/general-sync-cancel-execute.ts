...
export type LocalGeneralSyncCancellationExecutionResult =
  | {
  ...
  | {
      ...
      cancelledOwnJobs: number;
      untouchedWaitingJobs: number;
    };

const FINAL_BATCH_STATUSES = ["completed", "completed_with_errors", "failed", "cancelled"];
const CANCELLABLE_OWN_JOB_STATUSES = ["queued", "paused", "deferred"];

async function loadOwnedCancellingRun(client: PoolClient, runId: string, workerId: string) {
  const result = await clientQuery<RunRow>(
    client,
    `select id, requested_by, cancel_reason
...
export async function executeClaimedLocalGeneralSyncCancellation(input: {
  runId: string;
  workerId: string;
}): Promise<LocalGeneralSyncCancellationExecutionResult> {
  try {
    return await withTransaction(async (client) => {
      const run = await loadOwnedCancellingRun(client, input.runId, input.workerId);
      ...
      for (const batch of batches) {
        ...
        if (job && CANCELLABLE_OWN_JOB_STATUSES.includes(job.status)) {
          const cancelledJob = await clientQuery<{ id: string }>(
            client,
            `update processing_jobs
              ...
              returning id`,
            [job.id, CANCELLABLE_OWN_JOB_STATUSES]
          );

          if (cancelledJob.rowCount === 1) {
            cancelledOwnJobs += 1;
            ...
            [
              ...
              batch.status === "cancelled"
                ? batch.waiting_job_id
                  ? "Sincronizacao geral cancelada. O processamento externo ja existente nao foi interrompido."
                  : "Sincronizacao geral cancelada antes da conclusao deste lote."
                : null
            ]
          );
        }
    ...
    throw new DataAccessError(
      "Nao foi possivel executar o cancelamento da sincronizacao geral local.",
      "generalSyncCancel.execute",
      error
    );
  }
}