...
export type OperationalEvent = {
  id: string;
  operationType: "general_sync" | "individual_processing";
  title: string;
  source: "manual" | "scheduled" | "system" | null;
...
type OperationalEventRow = {
  ...
  success_items: number | string | null; error_items: number | string | null; last_error: string | null;
};
...
export async function getOperationalEvents(filters?: { campaignId?: string; batchId?: string; limit?: number; offset?: number }) {
  ...
  return ((data ?? []) as OperationalEventRow[]).map((row): OperationalEvent => ({
    id: row.id,
    operationType: row.operation_type === "general_sync" ? "general_sync" : "individual_processing",
    title: row.title,
    source: row.source === "manual" || row.source === "scheduled" || row.source === "system" ? row.source : null,
