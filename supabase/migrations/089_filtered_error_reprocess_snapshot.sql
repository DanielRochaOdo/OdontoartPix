...
begin
  ...
  -- O snapshot vira pending apenas no estado tecnico. Os campos financeiros
  -- permanecem com a ultima verdade conhecida ate a nova resposta do ERP.
      ...
      if v_run_batch_status in ('completed', 'completed_with_errors', 'failed') then
        update public.general_sync_run_batches
           set status = 'pending',
               ...
               waiting_job_id = null,
               processed_count = greatest(processed_count - v_batch.item_count, 0),
               error_count = greatest(error_count - v_batch.item_count, 0),
               error_reprocess_only = false,
               finished_at = null,
               message = 'Reprocessando snapshot filtrado de erros desta onda.',
               updated_at = v_now
         where id = v_run_batch_id;
      elsif v_dashboard_job_id is not null then
        update public.processing_jobs
           set total_items = greatest(total_items + v_batch.item_count, processed_items + v_batch.item_count),
               updated_at = v_now
