drop function if exists public.get_dashboard_metrics(uuid[], uuid[]);
...
as $$
  with selected_campaigns as (
    ...
    select
      ...
      coalesce(sum(cbm.installment_amount_cents) filter (where cbm.processing_status = 'completed' and cbm.payment_status = 'paid'), 0)::bigint as paid_amount
