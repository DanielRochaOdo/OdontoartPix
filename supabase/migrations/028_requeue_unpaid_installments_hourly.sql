...
begin
  ...
  with selected as (
    ...
    where cbm.batch_id = p_batch_id
      ...
      and (
        ...
        or (
          ...
          and coalesce(cbm.last_checked_at, cbm.updated_at, cbm.created_at) <= now() - interval '55 minutes'
        )
    ...
    order by
      case
        ...
        when cbm.processing_status = 'completed' and cbm.payment_status = 'unpaid' then coalesce(cbm.last_checked_at, cbm.updated_at, cbm.created_at)
  ...
  set
    ...
    last_reclaim_reason = case
      ...
      when selected.processing_status = 'completed' and selected.payment_status = 'unpaid' then 'scheduled-unpaid-recheck'
