-- Evita que cliques repetidos em Erros reenfileirem/descontem o mesmo registro
-- mais de uma vez antes de ele ser efetivamente reivindicado pelo worker.

create or replace function public.absorb_batch_errors_into_dashboard_v2(p_batch_id uuid)
returns table (
  absorbed boolean,
  run_id uuid,
  job_id uuid,
...
begin
  ...
  if v_count > 0 then
    ...
    values (
      ...
      'info',
      p_batch_id,
      'Erros novos solicitados para reprocessamento dentro da propria onda do dashboard.',
      jsonb_build_object(
        'runId', v_run_id,
        'jobId', v_job_id,
...
revoke all on function public.absorb_batch_errors_into_dashboard_v2(uuid) from public, anon, authenticated;
grant execute on function public.absorb_batch_errors_into_dashboard_v2(uuid) to service_role;

create or replace function public.request_member_reprocess_v2(
...
begin
  ...
  if v_run_id is not null then
    ...
    elsif v_dashboard_job_id is not null and v_is_new_error_request then
      update public.processing_jobs
         set include_errors = true,
  ...
  if v_existing_job.id is not null then
    update public.processing_jobs
       set include_errors = include_errors or v_is_new_error_request,
           total_items = greatest(total_items, processed_items + 1),
           updated_at = v_now
     where id = v_existing_job.id;
    return query select 'existing_job'::text, v_existing_job.id,
      v_existing_job.processing_priority, v_existing_job.processing_scope,
      v_member.batch_id, v_member.campaign_id, v_member.target_installment_id;
    return;
  end if;
  ...
  on conflict (batch_id, processing_origin) where status in ('queued', 'running')
  do nothing
...
revoke all on function public.request_member_reprocess_v2(uuid, uuid) from public, anon, authenticated;
grant execute on function public.request_member_reprocess_v2(uuid, uuid) to service_role;
