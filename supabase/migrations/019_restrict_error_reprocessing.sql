-- In error-reprocessing mode, claim only members already marked as error.
-- Normal processing continues to claim pending members only.
drop function if exists public.claim_batch_members(uuid, uuid, integer, boolean);
...
begin
  ...
  with selected as (
    ...
    where cbm.batch_id = p_batch_id
      ...
      and case
        when p_include_errors then cbm.processing_status = 'error'
        else cbm.processing_status in ('pending', 'pendente', 'aguardando')
      end
